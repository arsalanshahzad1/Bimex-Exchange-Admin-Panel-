...
class TestService
{
    ...
    {
        $this->api_key = (!empty($api_key)) ? $api_key : env("BROKER_API_KEY");
        $this->api_secret = (!empty($api_secret)) ? $api_secret : env("BROKER_SECRET");
        $this->api_url = (!empty($api_url)) ? $api_url : env("BINANCE_BASE_URL").'api/';
        $this->recvWindow = (!empty($timing)) ? $timing : 5000;
    }
