...
//                $this->checkErc20ContractAddress();
                ...
                // $token =  new CustomTokenRepository();
                // $token->depositCustomToken();
                $this->testTRC20Network();
                dd('executed');
            } catch (\Exception $e) {
                dd($e->getMessage());
            }
...
//        }
    ...
    public function checkErc20ContractAddress()
    {
        $coin = Coin::join('coin_settings', 'coin_settings.coin_id', '=', 'coins.id')
            ->where(['coins.coin_type' => 'BCE'])
            ->first();
        $api = new ERC20TokenApi($coin);
        $requestData = array(
//            "address" => '0x281480653F77506b070E1738EE8CbEaFeaB10145',
            "contract_address" => '0xc83Fc18806C85A3BDc37d0776EF93e9a20ff5994',
            "transaction_hash" => '0x780ea07306777656fe1e5af96c76661592914e45db7669c69792a46284e42190',
        );
...
// buy order create
    ...
    // test trc20 network
    public function testTRC20Network()
    {
        $coin_type = 'SDTRC';
        $coin = Coin::join('coin_settings', 'coin_settings.coin_id', '=', 'coins.id')
        ...
        $api = new ERC20TokenApi($coin);
        $address = 'TLNDtYheAhfLynRnE8i5c4J45h4kLyTuQN';
        $requestData = array(
            "amount_value" => 0.01,
            ...
            "type" => 2,
            "key" => "keeeee",
            "transaction_hash" => "d859fb13346ddb35e98b6d810424adff20efc83dce84b361dd33748672027256",
        );
        // $response = $api->checkWalletBalance($requestData);
        // $response = $api->getTrxAccount($requestData);
        // $response = $api->getTrxAddress($requestData);
        // $response = $api->checkTrxAddress($requestData);
        // $response = $api->sendEth($requestData);
        // $response = $api->getTransactionData($requestData);
        // $response = $api->getTrxConfirmedTransaction($requestData);
        $response = $api->sendCustomToken($requestData);
        // $response = $api->getContractTransferEvent();
        dd($response);
    }
}
